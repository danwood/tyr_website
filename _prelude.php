...
class Event
{
    function __construct($eventAssoc)
    {
    	global $now;
    	global $urlBase;
...
{
	if (!$originalEventAssocArray[BEFORE_AnnounceDate]) continue;

	$event = new Event($originalEventAssocArray);		// Copy the event, work with that.
	$events[] = $event;
...
//	error_log($event->title() . " state: " . $event->state);
