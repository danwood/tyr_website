	...
	if ($photoFilename)
	{
		...
		if ($matches)
		{
			...
			for ($i = 0 ; $i < $photoCount ; $i++)
			{
				...
				$movieInstead = (file_exists('../photo/' . $movieFile));
...
?>
										<video class="full-block" controls="controls" poster="<?php echo $root . 'photo/' . $base . ($i+1) . '.' . $extension; ?>">
											<source src="<?php echo $root; ?>photo/<?php echo $movieFile; ?>" />
										</video>
...
{
?>
										<img class="full-block" src="<?php echo $root . 'photo/' . $base . ($i+1) . '.' . $extension; ?>"
											alt="<?php echo htmlspecialchars($event->title() . ' photo ' . ($i+1)); ?>">


<?php
	if (file_exists('../photo/original.' . $base . ($i+1) . '.' . $extension ))
	{
?>
											<a href="<?php echo $root . 'photo/original.' . $base . ($i+1) . '.' . $extension; ?>"
...
<?php
	...
	{
?>
											<a href="<?php echo $root . 'photo/' . $base . ($i+1) . '.' . $extension; ?>"
												title="<?php echo htmlspecialchars($event->title()); ?>"
												rel="enclosure"></a>
...
?>
									<div class="inlinebox archive-photo">
										<img class="full-block" src="<?php echo $root; ?>photo/<?php echo $photoFilename; ?> ">
									</div>
<?php
		}
	}
